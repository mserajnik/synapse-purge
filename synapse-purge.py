...
def get_last_event_id(db: postgres.Postgres, room_id: str, before: datetime) -> Optional[str]:
    return db.one(
        'SELECT event_id FROM events WHERE received_ts <= %(before)s AND room_id=%(room_id)s ORDER BY received_ts DESC LIMIT 1',
        before=int(before.timestamp() * 1000), room_id=room_id)
...
def get_local_media_record_ids(db: postgres.Postgres, before: datetime) -> List[str]:
    return db.all('SELECT media_id FROM local_media_repository WHERE created_ts <= %(before)s',
...
def delete_local_media_record(db: postgres.Postgres, media_id: str) -> None:
    db.run('DELETE FROM local_media_repository WHERE media_id = %(media_id)s', media_id=media_id)


def purge_history(session: requests.Session, api_url: str, room_id: str, event_id: str) -> str:
...
def purge_history_status(session: requests.Session, api_url: str, purge_id: str) -> PurgeStatus:
...
def purge_remote_media(session: requests.Session, api_url: str, before: datetime) -> Dict:
    url = get_api_url(api_url, PURGE_REMOTE_MEDIA_API_ENDPOINT)
    return session.post(url, params={'before_ts': int(before.timestamp() * 1000)}).json()


def get_local_media_path(media_store_path: str, media_id: str) -> str:
    return os.path.join(media_store_path, 'local_content', media_id[:2], media_id[2:4], media_id[4:])


def wait_for_purge(session: requests.Session, api_url: str, purge_id: str, interval: float = 1) -> PurgeStatus:
    while True:
...
def main(arguments: argparse.Namespace) -> ExitCode:
    ...
    for index, room_id in enumerate(rooms, start=1):
        ...
        if event_id is None:
            logger.info('No event ID before: {} for room: {!r}, skipping', before_date_string, room_id)
            continue

        logger.info('Last event ID before: {} for room {!r}: {!r}', before_date_string, room_id, event_id)
    ...
    for index, media_id in enumerate(old_media, start=1):
        logger.info('({}/{}) Processing media: {!r}...', index, old_media_count, media_id)
